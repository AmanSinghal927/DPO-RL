...
def get_shp(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    for prompt in data:
        # setting the sft tarket to the response with higher rating only when the score ratio > 2
...
def get_batch_iterator(names: List[str],
    ...
    with TemporarilySeededRandom(seed):
        ...
        flat_data = []
        # names referes to the datasets here, e.g. hh is which dataset(s) to train on
        for name in names:
