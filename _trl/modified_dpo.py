...
class MMLUEvalCallback(WandbCallback):
    ...
    def on_evaluate(self, args, state, control, model, **kwargs):
        ...
        if accuracy>self.max_accuracy:
            ...
            save_path = os.path.join("/scratch/km6276/DPO-RL/drl/checkpoints", "lr_"+str(self.config_dict["lr"])+"_warmup_"+str(self.config_dict["warmup_steps"])+"_alpha_"+str(self.config_dict["beta"]))
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            model.save_pretrained(save_path)
...
def main(config):
    ...
    # wandb.init(project="medhalt_preference_optimization", name="beta_"+str(config.beta))
    wandb.init(project="medhalt_preference_optimization", name="lr_"+str(lr)+"_warmup_"+str(warmup_steps)+"_beta_"+str(config.beta))
