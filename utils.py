...
def get_open_port():
    """
    FSDP needs an open port for syncing data and data transfer
    """
...
def all_gather_if_needed(values: torch.Tensor, rank: int, world_size: int) -> torch.Tensor:
    ...
    dist.all_gather(all_values, values) # get all the values from all the gpus
    cat_function = torch.cat if values.dim() > 0 else torch.stack # NOTED
